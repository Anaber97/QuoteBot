...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (quote.bol_path) {
      const { data: bolFile, error: bolError } = await admin.storage.from('quote-bols').download(quote.bol_path);
      if (!bolError && bolFile) {
        bolAttachment = {
          filename: String(quote.bol_name || 'bill-of-lading').replace(/[^A-Za-z0-9._-]/g, '-').slice(0, 120),
          content: Buffer.from(await bolFile.arrayBuffer()).toString('base64'),
          content_type: quote.bol_type || bolFile.type || 'application/pdf',
        };
      }
    }
    ...
    const subjectPrefix = isDispatch ? 'Dispatch requested' : 'Your transport quote';
    ...
    const { data: emailData, error: emailError } = await admin.functions.invoke('send-quote-approval-email', {
      ...
      attachments: [{ filename: document.filename, content: document.bytes.toString('base64'), content_type: 'application/pdf' }, ...(bolAttachment ? [bolAttachment] : [])],
      },
    });
