...
export async function buildQuotePdf({ quote, company, config, logoBytes = null }) {
  ...
  y = 696; rule(); y -= 4;
  page.drawText(quote.min_quote === quote.max_quote ? money(quote.min_quote) : `${money(quote.min_quote)} - ${money(quote.max_quote)}`, { x: margin, y, size: 24, font: bold, color: ink });
