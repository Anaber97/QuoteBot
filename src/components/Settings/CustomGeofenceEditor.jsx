  ...
  return (
    <div className="space-y-3 rounded-xl border border-slate-800 bg-[#121824] p-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        <div>
          <label className="mb-1 block text-[10px] text-slate-400">Fee type</label>
          <select
            value={zone.feeType || 'percent'}
            onChange={(e) => onChange('feeType', e.target.value)}
            className="w-full rounded border border-slate-700 bg-[#080c14] p-2 text-white"
          >
            <option value="percent">Surcharge (%)</option>
      ...
      <div>
        ...
        <input
          ...
          placeholder={zone.feeType === 'flat' ? '75' : '25'}
          className="w-full rounded border border-slate-700 bg-[#080c14] p-2 text-white font-mono"
        />
        <p className="mt-1 text-[10px] text-slate-500">Flat rate applies when both pickup and dropoff are in the selected locality.</p>
      </div>
